...
export default function Statistics({ theme }) {
  const [locais, setLocais] = useState([]);
  const [latidesLongetudes, setLatitudeLongetude] = useState([
    ...
    <div className='flex justify-center items-center flex-col gap-6 relative'>
      <div className=''>
        <PopBar
          ...
          campo={"Home"}
          icon={<FiHome />}
          onClick={() => navigate(rotas[1].sub_destino)}
          theme={theme}
        />
      </div>
      <div className='pt-16'>
        <div className='flex justify-center flex-col items-center gap-2'>
          ...
          <p className='text-xl font-semibold'>
            Numeros de acessos: {locais2?.length}
          </p>
        </div>
      </div>
      <div className='flex flex-col-reverse gap-8 lg:mt-12 items-center max-w-4xl w-full'>
        <div className='flex-col gap-6 items-center w-full'>
          <div className='flex justify-center'>
            <h2 className='text-xl font-semibold'>Representação Grafica</h2>
          </div>
          <div className={`mt-6 estatisticas h-1/2`}>
            <Bar data={userData} options={options} />
          </div>
        </div>
        <div className='flex-col gap-6 items-center w-full px-3'>
          <div className='flex justify-center'>
